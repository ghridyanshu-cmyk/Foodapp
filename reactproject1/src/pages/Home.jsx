...
const Home = () => {
    ...
    return (
        <div className='bg-slate-200 w-full min-h-[100vh]'>
            ...
            <div className={`w-full md:w-[40vw] h-full fixed top-0 right-0 bg-white shadow-xl p-6 ${showCart ? "translate-x-0" : "translate-x-full"} transition-all duration-500 flex flex-col items-center overflow-auto`}>
                <header className='w-full flex justify-between items-center'>
                    <span className='text-green-400 text-[18px] font-semibold'>Order items</span>
                    <RxCross1 className='w-[30px] h-[20px] text-green-400 cursor-pointer hover:text-gray-600' onClick={() => setShowCart(false)} />
                </header>
                {items.length > 0 ? <>
                    <div className='w-full mt-9 flex flex-col gap-8'>
                        {items.map(item => <Card2 key={item.id} product={item} />)}
                    </div>
