    ...
    return (
        <div className="min-h-screen bg-gray-100 flex flex-col">
            <div className="relative pt-12 pb-24 bg-white shadow-md">
                <div className="absolute inset-x-0 top-0 h-40 bg-emerald-500 clip-bottom-wave"></div>
                <div className="relative mx-auto max-w-sm w-11/12 bg-white rounded-xl shadow-xl p-6 text-center z-10">
                    <img src={userData.profilePicUrl} alt="Profile Picture" className="w-20 h-20 rounded-full object-cover mx-auto -mt-16 border-4 border-white shadow-lg" onError={(e)=>{ e.target.onerror=null; e.target.src="https://placehold.co/100x100/10B981/ffffff?text=U"; }} />
            ...
            <div className="flex-grow p-4 space-y-4">
                ...
                <button onClick={()=>handleLogout()} className="w-full flex items-center justify-center space-x-3 p-4 bg-white text-red-500 rounded-xl shadow-md hover:bg-gray-100 transition font-semibold">
                    <LogOut className="w-5 h-5" />
                    <span>Logout</span>
                </button>
            </div>
            <EditProfileModal isOpen={isEditing} onClose={()=>setIsEditing(false)} userData={userData} onSave={handleSaveProfile} />
            <style jsx="true">{`.clip-bottom-wave { clip-path: polygon(0 0, 100% 0, 100% 75%, 50% 100%, 0 75%); }`}</style>
        </div>
    );
};
